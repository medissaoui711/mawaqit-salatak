...
const MawaqitApp: React.FC = () => {
  ...
  // Navigation State
  const [activeTab, setActiveTab] = useState<'home' | 'calendar' | 'athkar' | 'quran' | 'mosques' | 'chat'>('home');
    ...
    >
           ...
           >
             <div className="bg-zinc-800 border border-neon/50 text-white rounded-xl shadow-2xl p-4 flex items-center gap-4 max-w-sm w-full backdrop-blur-md">
                ...
                <button 
                  ...
                  className="bg-neon text-black px-4 py-2 rounded-lg text-xs font-bold hover:bg-neon/90"
                >
                  تحديث
                </button>
                <button onClick={closeUpdate} className="text-zinc-500 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
             </div>
           </motion.div>
      ...
      <header className="sticky top-0 z-40 bg-dark/80 backdrop-blur-lg border-b border-zinc-800/50 px-6 py-4 flex justify-between items-center">
         <div className="flex items-center gap-3">
            ...
            <div>
               ...
               <p className="text-zinc-400 font-medium text-xs leading-none flex items-center gap-2">
                 {city}
                 {isOffline && <WifiOff className="w-3 h-3 text-red-500" />}
               </p>
            </div>
         </div>
         <div className="flex items-center gap-2">
           {isInstallable && (
             <button onClick={installPWA} className="p-2 bg-neon/10 hover:bg-neon/20 rounded-full transition-colors text-neon" aria-label="Install App">
                <Download className="w-6 h-6" />
             </button>
           )}
           <button onClick={() => setIsChallengesOpen(true)} className="p-2 hover:bg-zinc-800 rounded-full transition-colors text-zinc-400 hover:text-yellow-400">
              <Trophy className="w-6 h-6" />
           </button>
           <button onClick={() => setIsSettingsOpen(true)} className="p-2 hover:bg-zinc-800 rounded-full transition-colors text-zinc-400 hover:text-neon">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-dark/90 backdrop-blur-xl border-t border-zinc-800 pb-safe z-50">
        <div className="flex justify-around items-center h-16 max-w-md mx-auto px-1">
           <button onClick={() => setActiveTab('home')} className={`flex flex-col items-center gap-1 p-2 ${activeTab === 'home' ? 'text-neon' : 'text-zinc-500'}`}>
             <Home className="w-5 h-5" /><span className="text-[9px] font-bold">{t('common.home')}</span>
           </button>
           <button onClick={() => setActiveTab('mosques')} className={`flex flex-col items-center gap-1 p-2 ${activeTab === 'mosques' ? 'text-neon' : 'text-zinc-500'}`}>
             <MapIcon className="w-5 h-5" /><span className="text-[9px] font-bold">{t('common.mosques')}</span>
           </button>
           <button onClick={() => setActiveTab('quran')} className={`flex flex-col items-center gap-1 p-2 ${activeTab === 'quran' ? 'text-neon' : 'text-zinc-500'}`}>
             <BookOpen className="w-5 h-5" /><span className="text-[9px] font-bold">{t('common.quran')}</span>
           </button>
           <button onClick={() => setActiveTab('athkar')} className={`flex flex-col items-center gap-1 p-2 ${activeTab === 'athkar' ? 'text-neon' : 'text-zinc-500'}`}>
             <span className="text-xl leading-none">📿</span>
             <span className="text-[9px] font-bold">{t('common.athkar')}</span>
           </button>
           <button onClick={() => setActiveTab('chat')} className={`flex flex-col items-center gap-1 p-2 ${activeTab === 'chat' ? 'text-neon' : 'text-zinc-500'}`}>
      ...
      <SettingsModal 
        ...
        onUpdateLocation={(newCoords, newCity) => { setCoords(newCoords); setCity(newCity); loadPrayerTimes(newCoords, displayDate); }}
      />

      {isChallengesOpen && (
          <Suspense fallback={null}>
            <ChallengesModal isOpen={isChallengesOpen} onClose={() => setIsChallengesOpen(false)} />
          </Suspense>
      )}
